...
#PBS -l select=1:ncpus=1:ngpus=1:mem=64gb:scratch_local=96gb:cl_elbi1=False:cl_elmu1=False:cl_eluo1=False:cl_elum1=False:cl_elwe=False:cl_elmo5=False:cl_elmo4=False:cl_eltu=False:cl_elmo3=False:cl_elmo2=False:cl_elmo1=False
#PBS -N best_model_anomaly_detection_gpu_torch_training
...
if [ -z ${seed+x} ]; then  # If the seed is not set
    ...
    singularity run --nv $CONTAINER python3 $MAIN_SCRIPT --market_id $market_id --date $date --market_segment_id $market_segment_id --security_id $security_id --model_type $model_type --epochs $epochs --kfolds $kfolds --batch_size $batch_size --lr $lr --seq_len $seq_len --latent_dim $latent_dim --seed $seed
fi
