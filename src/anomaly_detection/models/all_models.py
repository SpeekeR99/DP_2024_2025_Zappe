...
from src.anomaly_detection.models.autoencoder import FFNNAutoencoder, CNNAutoencoder, create_sequences, undo_sequences
...
def main():
    ...
    seq_len = 300
    data_tensor_seq = create_sequences(data_tensor, seq_len=seq_len)
    batch_size = 32
    ...
    model_cnn_ae = CNNAutoencoder(input_size=data_numpy.shape[1], latent_space_size=latent_dimensions)
    ...
    y_pred_cnnae, y_scores_cnnae, anomaly_proba_cnnae, em_val_cnnae, mv_val_cnnae, em_curve_cnnae, mv_curve_cnnae, t_cnnae, axis_alpha_cnnae, amax_cnnae = train_torch_model(model_cnn_ae, data_loader_seq, num_epochs=num_epochs, lr=lr_e_4, kfolds=kfolds, eval=True)
    ...
    model_names = ["Isolation Forest", "One-Class SVM", "Local Outlier Factor", "FFNN Autoencoder", "CNN Autoencoder"]
    short_model_names = ["IF", "OCSVM", "LOF", "FFNNAE", "CNNAE"]
    y_preds = [y_pred_if, y_pred_ocsvm, y_pred_lof, y_pred_ffnnae, y_pred_cnnae]
    anomaly_probas = [anomaly_proba_if, anomaly_proba_ocsvm, anomaly_proba_lof, anomaly_proba_ffnnae, anomaly_proba_cnnae]
    em_vals = [em_val_if, em_val_ocsvm, em_val_lof, em_val_ffnnae, em_val_cnnae]
    mv_vals = [mv_val_if, mv_val_ocsvm, mv_val_lof, mv_val_ffnnae, mv_val_cnnae]
    em_curves = [em_curve_if, em_curve_ocsvm, em_curve_lof, em_curve_ffnnae, em_curve_cnnae]
    mv_curves = [mv_curve_if, mv_curve_ocsvm, mv_curve_lof, mv_curve_ffnnae, mv_curve_cnnae]
    t = t_if
    axis_alpha = axis_alpha_if
    amax = max(amax_if, amax_ocsvm, amax_lof, amax_ffnnae, amax_cnnae)
